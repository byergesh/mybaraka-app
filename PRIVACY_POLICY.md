...
**Last Updated / Последнее обновление / Соңғы жаңарту:** December 21, 2025
...
#### 2.5 Generated statements (Excel/PDF exports)
...
#### 2.6 Technical data (device/IP/analytics)
...
### 3) AI assistant: what it can access and what we send to third parties

MyBaraka includes an AI assistant feature. To answer you, the AI assistant may use:

- **Your message content** (what you type).
- **Your preferred name** (to address you correctly).
- A **limited financial snapshot** used for context, which may include:
  - totals for today (income/expense),
  - your goals (name/current/target),
  - your active budgets (name/monthly budget).

To generate AI responses, we send the above information to our AI service provider (currently **OpenAI**) as a processing request. This means:

- **Sent to OpenAI**: your message + the limited snapshot described above (and other necessary request metadata).
- **Not intentionally sent** unless needed:
  - your full transaction history (unless you explicitly type it in your message),
  - your avatar image (unless you explicitly share it via message or a future feature),
  - OTP codes and password reset codes.

We store chat history in our database so you can see past messages. You can reset/delete your chat history using the in-app reset feature (when available).
...
- **AI assistant**: generate helpful financial guidance based on your requests and the limited snapshot.
...
- **With AI providers** (e.g., OpenAI) for AI assistant responses (see Section 3).
...
#### 2.5 Скачиваемые выписки (Excel/PDF)
...
#### 2.6 Технические данные (устройство/IP/аналитика)
- Мы **не используем** сервисы crash reporting.
- Мы **не логируем намеренно** IP‑адрес и идентификаторы устройства для целей трекинга.

### 3) AI‑ассистент: к чему имеет доступ и что передаём третьим лицам

В MyBaraka есть функция AI‑ассистента. Чтобы ответить вам, ассистент может использовать:

- **Текст вашего сообщения** (то, что вы вводите).
- **Ваше предпочитаемое имя** (чтобы обращаться к вам по имени).
- **Ограниченный финансовый «снимок»** для контекста, который может включать:
  - итоги за сегодня (доходы/расходы),
  - ваши цели (название/текущая/целевая сумма),
  - ваши активные бюджеты (название/месячный лимит).

Для генерации ответов мы отправляем указанную информацию нашему провайдеру AI (на данный момент **OpenAI**) как запрос на обработку. Это означает:

- **Отправляем в OpenAI**: ваше сообщение + ограниченный «снимок» выше (и минимальные служебные параметры запроса).
- **Не отправляем намеренно**, если это не нужно:
  - полную историю транзакций (если вы сами не вставите её в текст сообщения),
  - ваш аватар (если вы явно его не отправляете или если в будущем не появится такая функция),
  - OTP‑коды и коды восстановления пароля.

Мы сохраняем историю чата в нашей базе, чтобы вы могли видеть прошлые сообщения. Вы можете сбросить/удалить историю чата через функцию сброса (если доступна в приложении).
...
- **AI‑ассистента**: ответы и рекомендации по вашим запросам на основе ограниченного контекста.
...
- **AI‑провайдерам** (например, OpenAI) для работы AI‑ассистента (см. Раздел 3).
...
#### 2.5 Excel/PDF үзінді көшірмелері
...
#### 2.6 Техникалық деректер (құрылғы/IP/аналитика)
...
### 3) AI көмекші: қандай деректерге қол жеткізе алады және үшінші тарапқа не жіберіледі

AI көмекші жауап беру үшін мыналарды қолдана алады:
- **Сіз енгізген хабарлама мәтіні**.
- **Сіздің атыңыз** (сізге атымен жүгіну үшін).
- Контекст үшін **шектеулі қаржылық «снимок»**:
  - бүгінгі кіріс/шығыс қорытындылары,
  - мақсаттарыңыз (атауы/ағымдағы/мақсат сомасы),
  - белсенді бюджеттеріңіз (атауы/айлық лимиті).

AI жауаптарын шығару үшін біз осы ақпаратты AI провайдеріне (қазіргі уақытта **OpenAI**) өңдеу сұрауы ретінде жібере аламыз:
- **OpenAI‑ге жіберіледі**: сіздің хабарламаңыз + жоғарыдағы шектеулі «снимок».
- **Әдейі жіберілмейді** (қажет болмаса):
  - толық транзакция тарихы (егер сіз өзіңіз хабарламаға енгізбесеңіз),
  - аватар (егер өзіңіз жібермесеңіз немесе болашақта бөлек функция енгізілмесе),
  - OTP және құпиясөзді қалпына келтіру кодтары.

Чат тарихы дерекқорда сақталады. Қолданбада бар болса, чат тарихын «reset» арқылы өшіре аласыз.
...
- AI көмекшінің жауаптарын жасауға,
...
- қызмет көрсетушілермен (хостинг және деректерді сақтау инфрақұрылымы),
- AI провайдерімен (OpenAI) — AI көмекші үшін (3‑бөлім),
